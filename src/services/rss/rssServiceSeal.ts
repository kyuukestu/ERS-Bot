...
import { rssDBSeal } from '~/database/SQL/database-seal';
import { Client, TextChannel, EmbedBuilder } from 'discord.js';
import { initializeSQLDBSeal } from '~/database/SQL/database-seal';
import { THREAD_CONFIG } from '~/database/SQL/database-seal';

initializeSQLDBSeal();

const FEED_URL = 'https://www.rpnation.com/forums/-/index.rss';
const CHANNEL_ID = '1479329634792505508'; //TODO - fill with SEAL Channel
...
export class RSSServiceSeal {
	...
	async start() {
		...
		console.log('Seal RSS Service started');
		await this.pollFeed();
	...
	private async pollFeed() {
		...
		const threadRows = rssDBSeal
			.prepare('SELECT threadID, replyCount, pubDate FROM rss_feed_seal')
			.all() as ThreadRow[];
		...
		for (const item of items) {
			...
			if (!stored) {
				...
				rssDBSeal
					.prepare(
						'INSERT INTO rss_feed_seal (threadID, title, link, pubDate, replyCount) VALUES (?, ?, ?, ?, ?)',
					)
					.run(
						threadID,
						item.title ?? null,
			...
			rssDBSeal
				.prepare(
					'UPDATE rss_feed_seal SET title = ?, link = ?, pubDate = ?, replyCount = ? WHERE threadID = ?',
				)
				.run(
					item.title ?? null,
					item.link ?? null,
			...
			const row = rssDBSeal
				.prepare('SELECT tags FROM rss_feed_seal WHERE threadID = ?')
